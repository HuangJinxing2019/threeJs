...
// 开始加载
loadingManager.onStart = function (url, itemsLoader, itemTotal){
    console.log('开始加载事件：', url, itemsLoader, itemTotal)
    oProgress.innerHTML = `加载中0.0%`
}

// 加载进度
loadingManager.onProgress = function (url, itemsLoader, itemTotal){
    let value = (itemsLoader / itemTotal * 100).toFixed(2) + '%'
    console.log('加载进度事件：', url, itemsLoader, itemTotal, value)
    oProgress.innerHTML = `加载中${value}`
}

// 加载错误事件
loadingManager.onError = function (url, itemsLoader, itemTotal){
    console.log('加载错误事件：', url, itemsLoader, itemTotal)
}
...
// 置换贴图
let displacementTexture = textureLoader.load(new URL('./assets/textures/height.jpg', import.meta.url).href);
// 粗糙贴图
